...
//     add_meta_box( 'metabox-id', __('Metabox Title', 'themeWP'), 'callback_metaboxes', array('post','page'), 'normal', 'high', null );
//   }
...
if ( !function_exists('themeWP_add_meta_boxes') ):
  function themeWP_add_meta_boxes () {
    //7 parametros:
      // id para identificar el metabox
      // Titulo del Metabox
      // Callback con el contenido
      ...
      // Prioridad en la que se mostrará
      // Argumentos con callback
    add_meta_box( 'themeWP_meta_box', __('Información de la Raza', 'themeWP'), 'themeWP_metaboxes', 'post', 'normal', 'high', null );
  }
endif;
add_action('add_meta_boxes', 'themeWP_add_meta_boxes');
if ( !function_exists('themeWP_metaboxes') ):
  function themeWP_metaboxes ( $post ) {
    wp_nonce_field( basename(__FILE__), 'themeWP_nonce' );
?>
...
if ( !function_exists( 'themeWP_save_metaboxes' ) ):
  function themeWP_save_metaboxes( $post_id, $post, $update ) {
    if ( !isset( $_POST[ 'themeWP_nonce' ] ) || !wp_verify_nonce( $_POST[ 'themeWP_nonce' ], basename( __FILE__ ) ) )
      return $post_id;
...
add_action('save_post', 'themeWP_save_metaboxes', 10, 3);
?>